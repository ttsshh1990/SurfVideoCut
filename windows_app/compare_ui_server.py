...
class CompareHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            html_name = safe_filename(str(payload.get("html_name", "")), "input.html")
            docx_b64 = str(payload.get("docx_b64", ""))
            html_b64 = str(payload.get("html_b64", ""))

            if not docx_b64 or not html_b64:
            ...
            output_name = f"{Path(html_name).stem}__docx_diff_comments.pdf"
            output_path = run_dir / output_name
            summary_path = run_dir / "comparison_summary.json"

            docx_path.write_bytes(base64.b64decode(docx_b64))
            html_path.write_bytes(base64.b64decode(html_b64))
